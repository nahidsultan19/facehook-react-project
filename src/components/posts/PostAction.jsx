...
const PostAction = ({ post, commentCount }) => {
  ...
  return (
    <div className="flex items-center justify-between py-6 lg:px-10 lg:py-8">
      <button
        className="flex flex-center gap-2 text-xs font-bold text-[#B8BBBF] hover:text-white lg:text-sm"
        onClick={handleLike}
      >
        <img className="w-6" src={liked ? LikedIcon : LikeIcon} alt="Like" />
        {!liked && <span>Like</span>}
      </button>

      <button className="flex icon-btn space-x-2 px-6 py-3 text-xs lg:px-12 lg:text-sm">
